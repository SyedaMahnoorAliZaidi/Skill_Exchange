...
const Installation: React.FC = () => {
  ...
  useEffect(() => { fetchInstallationServices(); }, []);

  const fetchInstallationServices = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        ...
        params: { category: "Installation" }
      });
      setServices(response.data);
    } catch (err: any) {
      console.error("Error fetching installation services:", err);
      setError(err.response?.data?.message || "Failed to fetch installation services");
  ...
  return (
    <div className="nc-InstallationPage">
    <Header3/>
      <br/>
      <br/>
      <br/>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Professional Installation Services</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert installation professionals in your area. From appliances to electronics, get reliable installation solutions with our skilled technicians.
          </p>
        </div>
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <ButtonPrimary onClick={fetchInstallationServices} className="w-full">Refresh Services</ButtonPrimary>
            </div>
          </div>
        </div>
        <div className="mb-8">
          <HeaderFilter heading={`${filteredServices.length} Installation Services Available`}
            subHeading="Professional installation experts ready to serve you" onClickTab={() => {}} />
          {loading && (
            <div className="text-center py-12">
              <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
              <p className="mt-2 text-gray-600">Loading installation services...</p>
            </div>
          )}
          {error && (
            <div className="text-center py-12">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <ButtonPrimary onClick={fetchInstallationServices} className="mt-4">Try Again</ButtonPrimary>
              </div>
            </div>
          )}
          {!loading && !error && filteredServices.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Installation Services Found</h3>
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity ? "Try adjusting your search criteria or filters." : "No installation services are currently available in your area."}
                </p>
