...
const Stitching: React.FC = () => {
  ...
  useEffect(() => {
    fetchStitchingServices();
  }, []);

  const fetchStitchingServices = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Fetch stitching services by category
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        headers: { Authorization: `Bearer ${token}` },
        params: { category: "Stitching" }
      });

      // Use the filtered services directly from the API
      const stitchingServices = response.data;

      setServices(stitchingServices);
    } catch (err: any) {
      console.error("Error fetching stitching services:", err);
      setError(err.response?.data?.message || "Failed to fetch stitching services");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    <div className="nc-StitchingPage">
     <Header3/>
      <br/>
      <br/>
      <br/>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert tailors and stitching professionals in your area. From traditional wear to modern fashion, 
            get the perfect fit with our skilled artisans.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <ButtonPrimary 
                onClick={fetchStitchingServices}
                className="w-full"
              >
                Refresh Services
              </ButtonPrimary>
        ...
        <div className="mb-8">
          <HeaderFilter
            heading={`${filteredServices.length} Stitching Services Available`}
            subHeading="Professional tailors and stitching experts ready to serve you"
            onClickTab={() => {}}
          />
          ...
          {loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-gray-600">Loading stitching services...</p>
            </div>
          )}
          ...
          {!loading && !error && filteredServices.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Stitching Services Found</h3>
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity 
                    ? "Try adjusting your search criteria or filters."
                    : "No stitching services are currently available in your area."
                  }
                </p>
                {(searchTerm || selectedCity) && (
                  <ButtonPrimary 
