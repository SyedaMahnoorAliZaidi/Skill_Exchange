...
const Moving: React.FC = () => {
  ...
  const fetchMovingServices = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        ...
        params: { category: "Moving" }
      });
      setServices(response.data);
    } catch (err: any) {
      console.error("Error fetching moving services:", err);
      setError(err.response?.data?.message || "Failed to fetch moving services");
  ...
  const handleCardClick = (service: MovingService) => {
     ...
     <Header3/>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Professional Moving Services</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert movers and moving professionals in your area. From residential to commercial moves, 
            get reliable moving services with our experienced professionals.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <ButtonPrimary onClick={fetchMovingServices} className="w-full">Refresh Services</ButtonPrimary>
            </div>
          </div>
        </div>

        <div className="mb-8">
          <HeaderFilter heading={`${filteredServices.length} Moving Services Available`}
            subHeading="Professional movers and moving experts ready to serve you" onClickTab={() => {}} />

          {loading && (
            <div className="text-center py-12">
              <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
              <p className="mt-2 text-gray-600">Loading moving services...</p>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-12">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <ButtonPrimary onClick={fetchMovingServices} className="mt-4">Try Again</ButtonPrimary>
              </div>
            </div>
          )}
            ...
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                ...
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity ? "Try adjusting your search criteria or filters." : "No moving services are currently available in your area."}
                </p>
