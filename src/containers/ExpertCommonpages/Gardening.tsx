...
const Gardening: React.FC = () => {
  const [services, setServices] = useState<GardeningService[]>([]);
  ...
  useEffect(() => { fetchGardeningServices(); }, []);

  const fetchGardeningServices = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        ...
        params: { category: "Gardening" }
      });
      setServices(response.data);
    } catch (err: any) {
      console.error("Error fetching gardening services:", err);
      setError(err.response?.data?.message || "Failed to fetch gardening services");
  ...
  const handleCardClick = (service: GardeningService) => {
  ...
  return (
    <div className="nc-GardeningPage">
      <AdminHeader  />
      <br/>
      <br/>
      <br/>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Professional Gardening Services</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert gardeners and landscaping professionals in your area. From lawn care to garden design, 
            transform your outdoor space with our skilled gardening experts.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <ButtonPrimary onClick={fetchGardeningServices} className="w-full">Refresh Services</ButtonPrimary>
            </div>
          </div>
        </div>

        <div className="mb-8">
          <HeaderFilter heading={`${filteredServices.length} Gardening Services Available`}
            subHeading="Professional gardeners and landscaping experts ready to serve you" onClickTab={() => {}} />

          {loading && (
            <div className="text-center py-12">
              <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
              <p className="mt-2 text-gray-600">Loading gardening services...</p>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-12">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <ButtonPrimary onClick={fetchGardeningServices} className="mt-4">Try Again</ButtonPrimary>
              </div>
            </div>
          )}
          ...
          {!loading && !error && filteredServices.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Gardening Services Found</h3>
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity ? "Try adjusting your search criteria or filters." : "No gardening services are currently available in your area."}
                </p>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-center text-white mb-8">
          <p className="text-xl mb-6 opacity-90">Can't find what you're looking for? Contact us to connect with more stitching professionals.</p>
