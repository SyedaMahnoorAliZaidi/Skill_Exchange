...
const Electrical: React.FC = () => {
  const [services, setServices] = useState<ElectricalService[]>([]);
  ...
  const fetchElectricalServices = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Fetch electrical services by category
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        headers: { Authorization: `Bearer ${token}` },
        params: { category: "Electrical" }
      });

      // Use the filtered services directly from the API
      const electricalServices = response.data;

      setServices(electricalServices);
    } catch (err: any) {
      console.error("Error fetching electrical services:", err);
      setError(err.response?.data?.message || "Failed to fetch electrical services");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="nc-ElectricalPage">
      <AdminHeader  />
      <br/>
      <br/>
      <br/>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert electricians and electrical professionals in your area. From wiring to installations, 
            get safe and reliable electrical solutions with our certified technicians.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="text"
                placeholder="Search by service, description, or city..."
                value={searchTerm}
            ...
            <div className="flex items-end">
              <ButtonPrimary 
                onClick={fetchElectricalServices}
                className="w-full"
              >
                Refresh Services
              </ButtonPrimary>
        ...
        <div className="mb-8">
          <HeaderFilter
            heading={`${filteredServices.length} Electrical Services Available`}
            subHeading="Professional electricians and electrical experts ready to serve you"
            onClickTab={() => {}}
          />
          ...
          {loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-gray-600">Loading electrical services...</p>
            </div>
          )}
          ...
          {!loading && !error && filteredServices.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Electrical Services Found</h3>
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity 
                    ? "Try adjusting your search criteria or filters."
                    : "No electrical services are currently available in your area."
                  }
                </p>
                {(searchTerm || selectedCity) && (
                  <ButtonPrimary 
          ...
          {!loading && !error && filteredServices.length > 0 && (
            <div className="grid gap-6 md:gap-8 grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">
              {filteredServices.map((service, index) => {
                const imageUrl = getImageUrl(service.cover_image) || 
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-center text-white mb-8">
          <p className="text-xl mb-6 opacity-90">Can't find what you're looking for? Contact us to connect with more electrical professionals.</p>
