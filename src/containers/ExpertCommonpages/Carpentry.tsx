...
const Carpentry: React.FC = () => {
  const [services, setServices] = useState<CarpentryService[]>([]);
  ...
  const fetchCarpentryServices = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await axios.get("http://localhost:8000/api/services-by-category/", {
        ...
        params: { category: "Carpentry" }
      });

      setServices(response.data);
    } catch (err: any) {
      console.error("Error fetching carpentry services:", err);
      setError(err.response?.data?.message || "Failed to fetch carpentry services");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="nc-CarpentryPage">
      <AdminHeader />
      <br/>
      <br/>
      <br/>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Find expert carpenters and woodworking professionals in your area. From furniture making to repairs, 
            get quality carpentry solutions with our skilled artisans.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <ButtonPrimary onClick={fetchCarpentryServices} className="w-full">
                Refresh Services
              </ButtonPrimary>
            </div>
          </div>
        ...
        <div className="mb-8">
          <HeaderFilter
            heading={`${filteredServices.length} Carpentry Services Available`}
            subHeading="Professional carpenters and woodworking experts ready to serve you"
            onClickTab={() => {}}
          />
          ...
          {loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-gray-600">Loading carpentry services...</p>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-12">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <ButtonPrimary onClick={fetchCarpentryServices} className="mt-4">Try Again</ButtonPrimary>
              </div>
            </div>
          )}
          ...
          {!loading && !error && filteredServices.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No Carpentry Services Found</h3>
                <p className="text-gray-600 mb-4">
                  {searchTerm || selectedCity 
                    ? "Try adjusting your search criteria or filters."
                    : "No carpentry services are currently available in your area."
                  }
                </p>
                {(searchTerm || selectedCity) && (
                  <ButtonPrimary onClick={() => { setSearchTerm(""); setSelectedCity(""); }}>
                    Clear Filters
                  </ButtonPrimary>
                )}
              </div>
